# ADR-024: Record-Ingestion Data-Plane Enablement
...
Running those paths for the first time surfaced three latent gaps that green pods masked. They are tenant-agnostic; they break record ingestion in any tenant, not just the Microsoft one. The fourth gap on this path, the Cosmos SQL data-plane role, is covered by ADR-022.
...
- The full `storage create -> recordstopic -> indexer-queue -> indexer -> search` flow works, which is also the end-to-end proof of the indexer-queue Workload-Identity image (ADR-022).
