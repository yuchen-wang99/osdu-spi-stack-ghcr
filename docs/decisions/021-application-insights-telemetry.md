# ADR-021: Application Insights for Service Telemetry
...
ADR-002 (now ADR-019) gives cluster-level Managed Prometheus and Container Insights, which cover node and container metrics and logs. They do not carry the per-request, per-service application traces and dependency calls that `core-lib-azure` is wired to emit, and they do not satisfy the agent's startup expectation of a connection string.
